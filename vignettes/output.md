...
S. Abbott (1), J. Munday (1), J. Hellewell (1), CMMID Coronavirus
working group (1), S. Funk (1)
...
transmission event \[4\]. This has implications for the likely
...
duration of the animal-to-human crossover event in particular, and the
impact this has on the estimated level of human-to-human transmission.
We aimed to provide decision makers, and researchers, with probability
estimates for each scenario considered, along with estimates of the
reproduction number (R0) across all scenarios.
...
distribution - to account for overdisperion - with a dispersion of 0.16
(assuming SARs like dispersion) \[5\]. The mean number of cases
generated by each case (R0) was sampled from a uniform distribution once
...
(assuming SARS like) \[5\]. After the simulation of the branching
process reporting delays were added. The reporting delay was sampled
from either a normal, a Poisson, or a negative binomial distribution.
This was determined by fitting these distributions to the compiled
line-list of cases in China (*reference needed*) and selecting the best
fit using the Chi-squared statistic. If no good fit was determined -
using an arbitrary threshold of 0.05 - then the reporting delay was
instead sampled from directly from the line-list.

**Scenario analysis**

We simulated the branching process model 10,000 times for all
combinations of the following parameters: transmission event size (20,
40, 60, 80, 200), transmission event duration (7 days, 14 days, 21 days,
and 28 days), the mean of the serial interval (4 days, 8.4 days \[5\],
12)), and the upper bound of the R0 (1, 2, 3, 4). Parameter values used
in the scenario analysis were either assumptions based on the current
knowledge of the Wuhan outbreak or based on those used previously for
SARs \[5\]. We ran the model from the beginning of the outbreak for each
scenario until the 25th of January 2020. The start date was determined
by combining the duration of the transmission event with the date the
fish market in Wuhan - the source of the outbreak - closed (31st of
December 2019). Outbreak simulation was stopped if a sample exceeded
5000 cases by the end of the simulation period.
...
the 18th of January (200 cases with an assumed upper bound of 2000
...
the transmission event size and transmission event duration for every
combination of upper R0 bounds and mean serial intervals. We then used
heat-maps to identify which scenarios were more likely to represent the
underlying outbreak. We then reported the median, maximum, and minimum
R0s of samples accepted after conditioning on the observed data,
stratified by the transmission event size, transmission event duration
and the assumed mean serial interval.

**Implementation**

All analysis was carried out using R version 3.6.2 \[6\]. The branching
process model was implemented using the `bpmodels` package \[7\]. The
...
1 to the R0 estimates across all scenarios when compared to the SARs
like (8.4 days) serial interval. For the SARs like interval the most
likely scenario - with an event size of 200 and a duration of a week)
(Figure 1) - resulted in an estimated R0 of 2.4 (1.6 - 3.9) (Table 1).
Decreasing the event size increased the R0 to 3.1 (2 - 4) for a
transmission event with 100 cases. A longer transmission event resulted
in a decrease in R0 to 2.6 (1.6 - 4). The most likely scenario with a
mean serial interval of 12 days - with an outbreak size of 200 and a
duration of 14 days - resulted in an estimated R0 of 3.3 (2 - 4) (Table
...
<table>
<caption>Median, minimum, and maximum reproduction numbers of the Wuhan outbreak conditioned on case data from the 3rd, 18th and 25th of January - for scenarios with a mean serial interval of 8.4 (SARs like). Stratified by initial transmission event size and duration.</caption>
<thead>
<tr class="header">
...
<table>
<caption>Median, minimum, and maximum reproduction numbers of the Wuhan outbreak conditioned on case data from the 3rd, 18th and 25th of January - for scenarios with a mean serial interval of 12. Stratified by initial transmission event size and duration.</caption>
<thead>
<tr class="header">
<th style="text-align: left;">Transmission event size vs. Transmission event duration</th>
<th style="text-align: left;">7</th>
...
of 2.4 (1.6 - 3.9), whilst the most probable longer serial interval
scenarios resulted in an estimated R0 of 3.3 (2 - 4). Reducing the event
size led to estimates of the R0 increasing but also reduced the
proportion of samples accepted. Similarly, increasing the event duration
reduced the estimated R0 whilst decreasing the proportion of accepted
samples.
...
connected to the initial animal-to-human crossover event becomes
...
event size and R0. However, until more is known about the outbreak this
cannot be easily assessed and our scenarios represent our understanding
of the current state of knowledge. A previous study also looked at
varying the event size and the impact that this had on R0 estimates
using a branching process \[4\]. Our work builds on this by also looking
at event duration, including reporting delays, and conditioning across
multiple time points. For comparable scenarios our results were similar
to those previously published but we found that R0 estimates were highly
...
potential for bias using their expert knowledge or privately held data.
...
for the Wuhan outbreak has a R0 of less than 1, unless the size of the
transmission event was much greater than currently reported. We also
found that a large initial crossover event was likely with a relatively
...
7 Funk S. *Bpmodels: Analysing chain statistics using branching process
models*. 2020. <https://github.com/sbfnk/bpmodels>

8 Boettiger C. An introduction to Docker for reproducible research. *ACM
SIGOPS Operating Systems Review* 2015;**49**:71–9.
