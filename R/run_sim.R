...
#' @param delay_mean Numeric, the mean reporting delay
#' @param delay_sd Numeric, the sd of  the reporting delay
#'
#' @return
#' @export
...
run_sim  = function(n, n_length, mean_si, sd_si, R0, 
  ...
  sim$time <- sim$time + rnorm(length(sim$time), delay_mean, delay_sd) ## need to improve this
  sim <- sim[sim$time < tf, ]
