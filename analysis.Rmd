...
scenarios <- tidyr::expand_grid(
  ...
  ## Add the number of days in an event as a vector
  dplyr::mutate(event_day = purrr::map(event_duration, ~ 1:.)) %>% 
  ## Add a scenario id
...
scenario_sims <- scenarios %>% 
  ...
  dplyr::mutate(sims = list(
    ...
    ~ ringbp::wuhan_sim(n.cores = 1,
                        ...
                        cap_cases = 4500, 
                        cap_max_days = sampled_and_set_parameters$max_outbreak_length[.x] + event_duration - event_day, 
                        r0within = sampled_and_set_parameters$detected_R0[.x], 
                        r0Am = sampled_and_set_parameters$index_R0[.x],
                        overkkmiss = sampled_and_set_parameters$neg_bin_disp[.x],
                        overkk = 0.19, 
                        vefficacy = sampled_and_set_parameters$treatment_effectiveness[.x],
                        vuptake = sampled_and_set_parameters$treatment_uptake[.x],
                        ring.size = 100, 
                        time_to_protection = 10, 
                        incub_mean = sampled_and_set_parameters$incubation_mean[.x], 
                        incub_var = sampled_and_set_parameters$incubation_var[.x],
                        inf_mean = sampled_and_set_parameters$inf_mean[.x], 
                        inf_var = sampled_and_set_parameters$inf_var[.x], 
                        delay_shape = 2.4114166, 
                        delay_rate = 0.3261129,
                        time_to_isolation = 1) %>% 
      dplyr::mutate(
        sample = .x,
        indexR0 = sampled_and_set_parameters$index_R0[.x]
      )
  ))) %>% 
  dplyr::ungroup() %>% 
  tidyr::unnest(c("sims")) %>% 
  tidyr::unnest("sims") %>% 
  dplyr::group_by(scenario, sample, indexR0) %>% 
  dplyr::summarise_at(.vars = c("outbreak_n_clusters", "outbreak_size", "outbreak_size_all"), sum)
```
...
restricted_scenarios <- scenario_sims %>% 
  dplyr::filter(outbreak_size > lower_case_bound, outbreak_size < upper_case_bound)
```
# Summarise analysis
