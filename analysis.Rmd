...
scenario_sims <- scenarios %>% 
  ...
  dplyr::mutate(sims = list(
    ...
    ~ ringbp::wuhan_sim(n.cores = 1,
                        ...
                        cap_cases = 4500, 
                        cap_max_days = sampled_and_set_parameters$max_outbreak_length[.] - event_duration + event_day, 
  ...
  tidyr::unnest(c("sims")) %>% 
  tidyr::unnest("sims") %>% 
  dplyr:group_by(event_size, event_duration, case_detection_plus_isolation) %>% 
